...
class Input{
    ...
    * @return {Vec} the current position of the mouse.
    */
    ...
    * @return {Vec} the current movement of the mouse.
    */
...
Input.inner.mousePosition = new Vec([0,0]);
Input.inner.mousePositionDelta = new Vec([0,0]);
Input.inner.mousePositionDeltaBuffer = new Vec([0,0]);
...
Input.inner.initialize = function(){
    ...
    document.addEventListener('keydown', e => {
        ...
        if (key === 'f') {
            if (document.pointerLockElement === canvas) {
                ...
                Input.inner.mousePosition.x = e.pageX
                Input.inner.mousePosition.y = e.pageY
                
            } else {
                canvas.requestPointerLock();
                Input.inner.mousePosition.x = Input.inner.offsetX;
                Input.inner.mousePosition.y = Input.inner.offsetY;
            }
        }
    }, false);
    ...
    canvas.addEventListener("mousemove", function(e){
       // console.log(e);
        let x = e.pageX - Input.inner.offsetX; //Get X,y where the canvas's position is origin.
        let y = e.pageY - Input.inner.offsetY;
        Input.inner.mousePositionDeltaBuffer.x = e.movementX; //Difference since last mouse move
        Input.inner.mousePositionDeltaBuffer.y = e.movementY;
        Input.inner.mousePosition.x += e.movementX;
        Input.inner.mousePosition.y += e.movementY;
    });
